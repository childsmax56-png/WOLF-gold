...
export default function App() {
  ...
  const [favoriteKeys, setFavoriteKeys] = useState<{ songName: string, eraName: string, url: string, song?: Song }[]>(() => {
    if (typeof localStorage !== 'undefined') {
      const saved = localStorage.getItem('wolfgold_favorite_keys');
      if (saved) {
        try {
  ...
  useEffect(() => {
    if (typeof localStorage !== 'undefined') {
      localStorage.setItem('wolfgold_favorite_keys', JSON.stringify(favoriteKeys));
    }
  }, [favoriteKeys]);
  ...
  const [volume, setVolume] = useState(() => {
    ...
    if (typeof localStorage !== 'undefined') {
      try {
        const saved = localStorage.getItem('wolfgold_playback_state');
        if (saved) {
  ...
  useEffect(() => {
    if (typeof localStorage !== 'undefined' && currentSong && currentEra) {
      ...
      localStorage.setItem('wolfgold_playback_state', JSON.stringify(stateToSave));
    }
          ...
          try {
            ...
            if (savedSong && savedEraName) {
              ...
              if (eraToRestore) {
                ...
                const savedFavs = localStorage.getItem('wolfgold_favorite_keys');
                if (savedFavs) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([mainRes, localRes, sheetsRes, recentRes, recentTabRes]) => {
        ...
        applyLocalSongs(json, localRes.data);
        applyTrackerSheetSongs(json, sheetsRes.data);
        setData(json);
        // Map recent.csv rows
        ...
        <main className={`flex-1 overflow-y-auto relative scroll-smooth bg-[#0a0a0a] flex flex-col ${showPlayer ? 'pb-44 md:pb-28' : ''}`}>
          ...
          <div className="mt-auto px-6 py-8 text-center border-t border-white/5">
            <p className="text-[10px] text-white/30 leading-relaxed">
              WOLFGOLD does not host or hold any illegal files. All links are external and provided as-is for educational and archival purposes only.
            </p>
            <p className="text-[10px] text-white/30 leading-relaxed">
              WOLFGOLD 2026 © [V1.8.5]
            </p>
            ...
            <p className="text-[10px] text-white/30 leading-relaxed mt-1 space-x-3">
              ...
              <span>·</span>
              <a href="https://docs.google.com/spreadsheets/d/1CMwzf-YO7yoNr5d-dsAOGFfHnYfGwXSFh1glniNlrck/" target="_blank" rel="noopener noreferrer" className="text-[var(--theme-color)]/50 hover:text-[var(--theme-color)] transition-colors underline">Link For The Sheet</a>
            </p>
          </div>
        </main>
      </div>
          ...
          >
            ...
            >
              ...
              <div className="space-y-4 mb-8 text-sm text-white/70 leading-relaxed font-medium">
                <p>
                  The Last.fm API keys were recently reset. Please log out and log back in to reconnect your account.
                </p>
                <p>
            ...
            >
              ...
              <div className="space-y-4 mb-8 text-sm text-white/70 leading-relaxed">
                <ul className="space-y-4">
                  <li>
                    <strong className="text-white">Wolf Gold launched!</strong>
                    <br />The Tyler the Creator unreleased music tracker is now live. Welcome!
                  </li>
                </ul>
